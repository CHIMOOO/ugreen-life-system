...
npm run seed          # 写入演示数据（商品库 + 三期 + 当前期 + 账单）
npm run all           # 一键同时启动 后端(41131)/前台(41132)/后台(41133)
# 或分开启动：npm run server / npm run web / npm run admin
```

管理后台默认密码 `admin123`（在 `server/.env` 配置）。

> 生产部署见 [DEPLOY.md](DEPLOY.md)（Docker 单镜像，一条 `docker compose up -d --build`）。
> 后续二次开发先读 [CLAUDE.md](CLAUDE.md)（架构契约与扩展速查）。

## 主要功能

- **抽奖**：姓名 + 1-9999 幸运数字（双防重复），取余算法公开可复现；结果页展示逐步推导。开奖后后台可**判定某人无效，名次自动顺延**。
- **下午茶评分**：全局商品库 + 每期录入；三档评分、好评率 =（推荐+还行）÷总票数；每浏览器每商品限一票。商品有内部字段（渠道/价格/数量），是否对用户展示由系统开关控制。
- **账单**：总账单流水（收入/支出/结余/垫付，详情页固定学院风），每期「本期账单」可由商品实际金额自动计算或手填。
- **12 套页面风格**（含「随机」）+ 首页风格模式（跟随当期/随机/固定）。
- **模块开关**：系统级 + 期数级分别开关抽奖/下午茶/账单。
- **后台可配**：部门名、系统名、规则、姓名输入框占位（趣味设置）、各类开关等。
- **看图录入商品**：`.claude/skills/tea-product-import/SKILL.md`，截图给 AI 即可批量录入。
