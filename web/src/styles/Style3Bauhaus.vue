  ...
  <div class="relative min-h-screen overflow-hidden bg-bau-bg font-outfit text-bau-ink">
    ...
    <div class="relative z-10 mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
        ...
        <div v-if="showForm" class="space-y-10">
          <div class="mx-auto w-full max-w-2xl">
            <div class="relative border-4 border-bau-ink bg-white p-6 shadow-bau-lg sm:p-10">
              ...
              <p class="mt-3 font-medium leading-relaxed text-bau-ink/70">实名或匿名都行——匿名请用工号当幸运数字。开奖前你的信息对所有人保密。</p>
                ...
                <div>
                  ...
                  <div class="flex items-center gap-3">
                    <input v-model="number" type="number" min="1" max="9999" placeholder="例如：888"
                      class="min-w-0 flex-1 rounded-none border-4 border-bau-ink bg-bau-bg px-5 py-4 text-lg font-bold text-bau-ink placeholder-bau-ink/30 outline-none transition focus:bg-white focus:shadow-bau-sm" />
                    <DiceButton @roll="(n) => (number = n)" class="shrink-0 text-bau-red" />
                  </div>
                </div>
      ...
      <section v-if="teaOn" class="mt-12 border-4 border-bau-ink bg-bau-red p-6 text-white shadow-bau-lg sm:p-10">
        ...
        <div v-if="period.bill && period.bill.show && period.bill.items.length" class="mt-6 border-4 border-bau-ink bg-bau-yellow p-5 text-bau-ink shadow-bau-md">
          <p class="text-sm font-black uppercase tracking-tight sm:text-base">📒 本期账单 · 合计 ¥{{ period.bill.total }}</p>
          <div class="mt-3 flex flex-wrap gap-2">
            <span v-for="b in period.bill.items" :key="b.id" class="border-2 border-bau-ink bg-white px-3 py-1 text-xs font-bold uppercase tracking-wide">{{ b.title }} · ¥{{ b.amount }}</span>
          </div>
        </div>
        <div class="mt-8 grid gap-6 sm:grid-cols-2">
          <div v-for="(prod, pi) in period.tea.products" :key="prod.id" class="overflow-hidden border-4 border-bau-ink bg-white text-bau-ink shadow-bau-md transition-transform duration-200 ease-out hover:-translate-y-1">
            <button v-if="prod.image" type="button" @click="openZoom(assetUrl(prod.image))" class="block w-full cursor-zoom-in border-b-4 border-bau-ink">
              <img :src="assetUrl(prod.image)" class="h-44 w-full object-cover transition hover:brightness-105" :alt="prod.name" />
            </button>
            <div v-else class="grid h-44 w-full place-items-center border-b-4 border-bau-ink text-6xl" :style="{ backgroundColor: primary(pi) }">🍰</div>
            <div class="p-5">
            <div class="min-w-0">
      ...
      <section v-if="config.lotteryModuleEnabled !== false || config.teaModuleEnabled !== false" class="mt-12">
        ...
        <div class="mt-8 grid gap-6 sm:grid-cols-2">
          <div v-if="config.lotteryModuleEnabled !== false" class="border-4 border-bau-ink bg-white p-6 shadow-bau-md">
            ...
            <Markdown :source="config.rulesLottery" class="mt-4 font-medium leading-relaxed text-bau-ink/80" />
          </div>
          <div v-if="config.teaModuleEnabled !== false" class="border-4 border-bau-ink bg-white p-6 shadow-bau-md">
            ...
            <Markdown :source="config.rulesTea" class="mt-4 font-medium leading-relaxed text-bau-ink/80" />
          </div>
        </div>
      </section>
