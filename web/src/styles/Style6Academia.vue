    ...
    <div class="mx-auto max-w-4xl px-5 py-8 sm:px-8">
      ...
      <section v-if="lotteryOn" class="mb-16">
        ...
        <div v-if="showForm" class="space-y-10">
          <div class="mx-auto w-full max-w-2xl">
                ...
                <div>
                  ...
                  <div class="flex items-center gap-3">
                    <input v-model="number" type="number" min="1" max="9999" placeholder="例如：888"
                      class="min-w-0 flex-1 w-full border-b-2 border-aca-ink bg-transparent px-1 py-2 font-garamond text-lg text-aca-ink placeholder-aca-brown/50 outline-none transition focus:border-aca-burgundy" />
                    <DiceButton @roll="(n) => (number = n)" class="shrink-0 text-aca-burgundy" />
                  </div>
                </div>
            ...
            <ConfirmSubmitDialog :open="showConfirm" :rules="config.rulesLottery" :submitting="submitting"
              :name="pending?.name" :number="pending?.number" :accent="'#7c2d12'"
              title="呈递卷宗" icon="❦" confirmText="呈递卷宗" cancelText="再想想"
              subText="请核对卷宗信息并研读规则，呈递之后，开奖前对外人保密。"
              :theme="{
                overlay: 'bg-black/70 backdrop-blur-sm',
                panel: 'aca-frame bg-aca-panel text-aca-ink font-garamond',
                heading: 'font-playfair text-2xl font-bold italic text-aca-ink',
                sub: 'italic text-aca-brown',
                infoBox: 'border-2 border-aca-gold/50 bg-aca-paper',
                infoLabel: 'font-playfair italic text-aca-brown',
                infoName: 'text-aca-ink',
                rulesLabel: 'font-playfair not-italic text-aca-burgundy',
                rulesBox: 'border-l-4 bg-aca-paper text-aca-ink/90',
                cancelBtn: 'border-2 border-aca-ink text-aca-ink hover:bg-aca-ink hover:text-aca-paper',
                confirmBtn: 'bg-aca-burgundy font-playfair uppercase tracking-widest text-aca-paper hover:bg-aca-ink',
              }"
          ...
          <div>
            ...
            <div class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <div v-for="(z, i) in period.prizes" :key="i" class="overflow-hidden bg-aca-panel aca-frame">
                ...
                <div v-else class="grid aspect-video w-full place-items-center border-b-2 border-aca-ink bg-aca-paper text-6xl">🎁</div>
        ...
        <div v-else-if="isDrawn && result" class="space-y-14">
            ...
            <div class="mt-8 grid gap-5 sm:grid-cols-2">
              <div v-for="(g, gi) in prizeGroups" :key="gi" class="border-2 border-aca-gold bg-aca-panel p-6 shadow-[0_0_0_1px_#B08D57_inset]">
                <div class="flex items-center gap-4">
                  <div class="aspect-video w-24 shrink-0 overflow-hidden border-2 border-aca-gold">
          ...
          <div v-if="skipped.length" class="aca-frame border-l-4 border-aca-burgundy bg-aca-panel p-6 sm:p-8">
            <h3 class="font-playfair text-2xl font-bold italic text-aca-burgundy">⚖ 无效顺延</h3>
            <p class="mt-2 leading-relaxed text-aca-brown">下列参与者因<b class="text-aca-burgundy">重复提交 / 冒用姓名等被判无效</b>，已取消其中奖资格；空出的名额按规则由后一位<b class="text-aca-forest">顺延递补</b>（所以最终名单里出现了原本靠后的人）。</p>
            <div class="mt-4 flex flex-wrap gap-2">
              <span v-for="(s, si) in skipped" :key="si" class="rounded-full border border-aca-burgundy/50 bg-aca-paper px-4 py-2 font-garamond font-semibold text-aca-ink">
                {{ s.name }} · <span class="font-playfair italic text-aca-burgundy line-through">{{ s.number }}</span>
              </span>
            </div>
          </div>
