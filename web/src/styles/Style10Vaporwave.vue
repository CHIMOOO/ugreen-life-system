    ...
    <div class="relative z-10 mx-auto max-w-5xl px-5 py-8 sm:px-8">
      ...
      <section v-if="lotteryOn" class="mb-16">
                ...
                <div>
                  ...
                  <div class="flex items-center gap-3">
                    <input v-model="number" type="number" min="1" max="9999" placeholder="例如：888"
                      class="min-w-0 flex-1 rounded-xl border border-vapor-pink bg-black/40 px-5 py-4 text-lg font-bold text-vapor-pink placeholder-vapor-pink/40 outline-none transition focus:border-vapor-cyan focus:vapor-glow" />
                    <DiceButton @roll="(n) => (number = n)" class="shrink-0 text-vapor-pink" />
                  </div>
                </div>
        ...
        <div v-else-if="isDrawn && result" class="space-y-12">
          ...
          <div v-if="skipped.length" class="rounded-2xl border border-vapor-pink/60 bg-vapor-pink/10 p-6 vapor-glow backdrop-blur sm:p-8">
            <h3 class="font-orbitron text-2xl font-black uppercase tracking-wider text-vapor-pink vapor-chrome">⚖ 无效顺延</h3>
            <p class="mt-2 leading-relaxed text-vapor-fg/80">下列参与者因<b class="text-vapor-pink">重复提交 / 冒用姓名等被判无效</b>，已取消其中奖资格；空出的名额按规则由后一位<b class="text-vapor-cyan">顺延递补</b>（所以最终名单里出现了原本靠后的人）。</p>
            <div class="mt-4 flex flex-wrap gap-2">
              <span v-for="(s, si) in skipped" :key="si" class="rounded-full border border-vapor-pink/60 bg-black/40 px-4 py-2 font-bold text-vapor-fg/85">
                {{ s.name }} · <span class="font-orbitron text-vapor-pink line-through">{{ s.number }}</span>
              </span>
            </div>
          </div>
      ...
      <section v-if="teaOn" class="mb-16">
        ...
        <div v-if="period.bill && period.bill.show && period.bill.items.length" class="mt-5 rounded-2xl border border-vapor-cyan/30 bg-vapor-panel/40 p-5 backdrop-blur vapor-glow">
          <p class="font-mono text-sm font-black uppercase vapor-chrome">📒 本期账单 · 合计 ¥{{ period.bill.total }}</p>
          <div class="mt-3 flex flex-wrap gap-2">
            <span v-for="b in period.bill.items" :key="b.id" class="rounded-full border border-vapor-pink/30 bg-black/30 px-3 py-1 text-sm text-vapor-fg/70">{{ b.title }} · ¥{{ b.amount }}</span>
          </div>
        </div>
        <div class="mt-6 grid gap-5 sm:grid-cols-2">
          <div v-for="(prod, pi) in period.tea.products" :key="prod.id" class="overflow-hidden rounded-2xl border bg-vapor-panel/60 vapor-glow backdrop-blur" :style="{ borderColor: accent(pi) }">
